...
def init_db():
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if not exists:
            conn.execute(
                'INSERT INTO staff_users(username, password, display_name, created_at) VALUES (?, ?, ?, ?)',
                (admin_user, admin_password, 'Salon Karola Admin', datetime.now().isoformat(timespec='seconds')),
            )
        mail_tpl = conn.execute("SELECT 1 FROM _MailTemplates WHERE id='appointment'").fetchone()
        if not mail_tpl:
            conn.execute(
        ...
        birth_tpl = conn.execute("SELECT 1 FROM _MailTemplates WHERE id='birthdate'").fetchone()
        if not birth_tpl:
            conn.execute(
                'INSERT INTO _MailTemplates(id, subject, body) VALUES (?, ?, ?)',
                (
...
def upcoming_birthdays(limit=12):
    rows = get_db().execute(
        "SELECT * FROM _Customers WHERE _birthdate IS NOT NULL AND _birthdate <> '' ORDER BY strftime('%m-%d', _birthdate) ASC LIMIT ?",
        (limit,),
    ).fetchall()
    return rows
...
def login():
    if request.method == 'POST':
        ...
        db = get_db()
        user = db.execute('SELECT * FROM staff_users WHERE username = ? AND password = ?', (username, password)).fetchone()
        if user:
...
def index():
    ...
    if q:
        like = f'%{q}%'
        conditions.append('(c._name LIKE ? OR c._firstname LIKE ? OR c._mail LIKE ? OR c.Customer_Mobiltelefon LIKE ? OR c.Customer_PersönlichesTelefon LIKE ? OR c.Customer_Stadt LIKE ?)')
...
def customer_detail(customer_id):
    ...
    appointments = db.execute(
        'SELECT * FROM appointments WHERE customer_id = ? ORDER BY appointment_at DESC', (customer_id,)
    ).fetchall()
    logs = db.execute(
        'SELECT * FROM email_log WHERE customer_id = ? ORDER BY sent_at DESC LIMIT 25', (customer_id,)
    ).fetchall()
    tags_text = ', '.join(r['tag'] for r in db.execute('SELECT tag FROM customer_tags WHERE customer_id = ? ORDER BY tag', (customer_id,)).fetchall())
    return render_template(
        'customer_form.html',
        customer=customer,
...
def templates_view():
    ...
    templates = {r['id']: r for r in db.execute("SELECT * FROM _MailTemplates WHERE id IN ('birthdate','appointment')").fetchall()}
...
def send_test(customer_id, template_id):
    ...
    appointment = db.execute('SELECT * FROM appointments WHERE customer_id = ? ORDER BY appointment_at ASC LIMIT 1', (customer_id,)).fetchone()
...
def export_customers():
    rows = get_db().execute(
        'SELECT * FROM _Customers ORDER BY _name, _firstname'
    ).fetchall()
    output = io.StringIO()
...
def boot_app():
    ...
    if not scheduler.running:
        scheduler.add_job(scheduler_tick, 'interval', minutes=interval_minutes, id='automation_loop', replace_existing=True)
        scheduler.start()
    ...
    try:
        ...
        if '_Customers' in src_tables:
            for row in src.execute('SELECT * FROM _Customers'):
                keys = row.keys()
                key = (((row['_firstname'] if '_firstname' in keys else '') or '').strip().lower(), ((row['_name'] if '_name' in keys else '') or '').strip().lower(), ((row['_mail'] if '_mail' in keys else '') or '').strip().lower())
        ...
        if 'appointments' in src_tables and '_Customers' in src_tables:
            ...
            for row in src_customers:
                key = (((row['_firstname'] or '').strip().lower()), ((row['_name'] or '').strip().lower()), ((row['_mail'] or '').strip().lower()))
                dest_customer = dest.execute('SELECT _id FROM _Customers WHERE lower(COALESCE(_firstname, "")) = ? AND lower(COALESCE(_name, "")) = ? AND lower(COALESCE(_mail, "")) = ? LIMIT 1', key).fetchone()
                if dest_customer:
            ...
            for row in src.execute('SELECT * FROM appointments'):
                ...
                if not mapped_customer:
                    continue
                duplicate = dest.execute('SELECT 1 FROM appointments WHERE customer_id = ? AND appointment_at = ? AND title = ? LIMIT 1', (mapped_customer, row['appointment_at'], row['title'])).fetchone()
                if duplicate:
                    continue
                dest.execute('INSERT INTO appointments(customer_id, title, appointment_at, notes, reminder_hours, reminder_sent_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)', (mapped_customer, row['title'], row['appointment_at'], row['notes'], row['reminder_hours'], row['reminder_sent_at'], row['created_at']))
    ...
    if request.method == 'POST':
        ...
        try:
            if action == 'merge':
                ...
                flash(f"Datenbank zusammengeführt. Neue Kontakte: {merged['customers']}, neue Termine: {merged['appointments']}, neue Vorlagen: {merged['templates']}. Backup: {backup_path.name if backup_path else 'keins'}")
            else:
                backup_path, info_after = replace_database_from_upload(file)
                flash(f"Datenbank komplett ersetzt. Aktuelle Kontakte: {info_after['counts'].get('_Customers', 0)}. Backup: {backup_path.name if backup_path else 'keins'}")
        except Exception as exc:
