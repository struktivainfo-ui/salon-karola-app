...
if not _secret_key:
    if production_like_runtime():
        raise RuntimeError(
            ...
            "Bitte in Render unter Environment Variables eintragen."
        )
    _secret_key = "dev-only-insecure-key-change-me"
    import warnings
    warnings.warn("SECRET_KEY nicht gesetzt — nur für lokale Entwicklung!", stacklevel=2)
...
def is_admin_staff_name(staff_name, db=None):
    ...
    conn = db or get_db()
    row = conn.execute(
        "SELECT is_admin FROM staff_users WHERE staff_name = ? LIMIT 1",
        (normalized,),
    ).fetchone()
...
def get_setting(key, default=""):
    try:
        ...
        conn = _g.db if "db" in _g else sqlite3.connect(DB_PATH)
...
def set_setting(key, value):
    try:
        from flask import g as _g
        conn = _g.db if "db" in _g else None
        if conn:
            conn.execute(
                """
...
def init_db():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        add_column_if_missing(
            ...
            fill_sql=f"UPDATE staff_users SET is_admin = CASE WHEN staff_name IN ({admin_staff_names_sql}) THEN 1 ELSE 0 END",
        )
        conn.execute(
            f"UPDATE staff_users SET is_admin = CASE WHEN staff_name IN ({admin_staff_names_sql}) THEN 1 ELSE 0 END"
        )
...
def ensure_default_admin(force_reset=False):
    with sqlite3.connect(DB_PATH) as conn:
        ...
        rows = conn.execute("SELECT id, username, password, display_name, staff_name, is_admin FROM staff_users").fetchall()
        for row in rows:
            stored_password = row[2] or ""
            desired_staff = row[4] or ""
            desired_is_admin = 1 if desired_staff in ADMIN_STAFF_NAMES else 0
            ...
            if desired_is_admin != int(row[5] or 0):
                updates.append("is_admin = ?")
                params.append(desired_is_admin)
            if updates:
                params.append(row[0])
        ...
        for account in staff_account_configs():
            ...
            if existing:
                if force_reset and account["password"]:
                    conn.execute(
                        "UPDATE staff_users SET username = ?, password = ?, display_name = ?, staff_name = ?, is_admin = ? WHERE id = ?",
                        (account["username"], hash_password(account["password"]), account["display_name"], account["staff_name"], 1 if account["staff_name"] in ADMIN_STAFF_NAMES else 0, existing[0]),
                    )
                    ...
                    if duplicate_username:
                        conn.execute(
                            "UPDATE staff_users SET display_name = ?, staff_name = ?, is_admin = ? WHERE id = ?",
                            (account["display_name"], account["staff_name"], 1 if account["staff_name"] in ADMIN_STAFF_NAMES else 0, existing[0]),
                        )
                    else:
                        conn.execute(
                            "UPDATE staff_users SET username = ?, display_name = ?, staff_name = ?, is_admin = ? WHERE id = ?",
                            (account["username"], account["display_name"], account["staff_name"], 1 if account["staff_name"] in ADMIN_STAFF_NAMES else 0, existing[0]),
                        )
                continue

            conn.execute(
                """
                INSERT INTO staff_users(username, password, display_name, staff_name, is_admin, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    account["username"],
                    ...
                    account["staff_name"],
                    1 if account["staff_name"] in ADMIN_STAFF_NAMES else 0,
